...
class EK60(object):
    ...
    def _read_datagram(self, fid, nmea=True):
        ...
        if dgram_header['type'].startswith('RAW'):
            ...
            # Check if this is a new "ping" or just a different channel in
            # the same ping.
        ...
        if new_datagram['type'].startswith('RAW'):
            ...
            for raw_obj in self.raw_data[new_datagram['channel_id']]:
                ...
                if raw_obj.data_type == '':
                    ...
                    this_raw_data = raw_obj
                    break
                #  mode 0 data is "no data" and theoretically can be any type so we can't use mode
                #  to determine type and need to see what is in the datagram instead.
                elif new_datagram['mode'] == 0:
                    if (new_datagram['power'] is not None and new_datagram['angle'] is not None and
                            raw_obj.data_type == 'power/angle'):
                        this_raw_data = raw_obj
                        break
                    if new_datagram['power'] is not None and raw_obj.data_type == 'power':
                        this_raw_data = raw_obj
                        break
                    if new_datagram['angle'] is not None and raw_obj.data_type == 'angle':
                        this_raw_data = raw_obj
                        break
