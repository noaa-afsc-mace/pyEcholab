            ...
            if new_datagram['channel_id'] in self.channel_ids:
                ...
                if new_datagram['type'][3] in ['0', '3']:
                    ...
                    for raw_obj in self.raw_data[new_datagram['channel_id']]:
                        ...
                        #  data_type 0 data is "no data" and theoretically can be any type so we can't use it
                        #  to determine type and need to see what is in the datagram instead. It is unknown at this
                        #  time if EK/ES80 software generates data_type==0 data but this is here for completeness.
                        elif new_datagram['data_type'] == 0:
                            if (new_datagram['power'] is not None and new_datagram['angle'] is not None and
                                    raw_obj.data_type == 'power/angle'):
                                this_raw_data = raw_obj
                                break
                            elif new_datagram['power'] is not None and raw_obj.data_type == 'power':
                                this_raw_data = raw_obj
                                break
                            elif new_datagram['angle'] is not None and raw_obj.data_type == 'angle':
                                this_raw_data = raw_obj
                                break
                            else:
                                #  this must be complex data? we're getting WAY out in the weeds here - no idea if
                                #  the EK/ES80 systems can generate data_type==0 complex data.
                                if new_datagram['n_complex'] == raw_obj.complex.shape[2]:
                                    is_fm = self._tx_params[new_datagram['channel_id']]['pulse_form'] > 0
                                    if is_fm and raw_obj.data_type == 'complex-FM':
                                        this_raw_data = raw_obj
...
class raw_data(ping_data):
    ...
    def get_power(self, calibration=None, **kwargs):
        ...
        If calibration is set to an instance of EK80.ek80_calibration the
        ...
        Args:
            ...
            calibration (EK80.ek80_calibration): Set to an instance of
                ...
                you want to use when transforming to power. If no calibration
    ...
    def get_electrical_angles(self, return_depth=False, calibration=None, **kwargs):
        ...
        Args:
            ...
            calibration (EK80.ek80_calibration): Set to an instance of
                EK80.ek80_calibration containing the calibration parameters
                you want to use when transforming to electrical angles. If no
                calibration object is provided, the values will be extracted from
                the raw data.
...
# as the data would be in different channels.
    ...
    def _get_sample_data(self, property_name, calibration=None,
        ...
        # there is only one reference. When requesting angle data, there will be two.
    ...
    def _get_transducer_offset(self, cal_parms):
        ...
        if (cal_parms['transducer_mounting'] is not None and
            ...
            # includes the individual transducer z offsets + the initial drop
        ...
        #  again for angle sensitivities
            ...
            if self.transceiver_type != 'GPT':
                ...
                if hasattr(raw_data, 'effective_pulse_duration'):
                    #  this must be an old EK80/ES80 raw file that has this in the file
